...
func GetOrders(db *sql.DB, start, count int) ([]Order, error) {
	rows, err := db.Query(
		  `Select *, count(*) OVER() AS full_count from (Select (NULLIF(it.price_per_unit, '')::decimal * it.quantity::decimal) as TotalAmount, it.quantity as Quantity, dl.delivered_quantity as DeliveredQuantity,
		  ...
		  join companies co on co.company_id = cu.company_id) tbl  LIMIT $1 OFFSET $2`,
		  count, start)
  if err != nil {
		  return nil, err
	  }
